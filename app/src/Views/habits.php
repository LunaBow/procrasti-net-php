<?php $title = 'Habits - Procrasti-php'; ?>
            ...
            <?php foreach ($habits as $h): ?>
                <article class="card" style="padding: 1.5rem; background: rgba(255, 255, 255, 0.03); border-radius: var(--radius-md); border: 1px solid var(--border); display: flex; flex-direction: column; justify-content: space-between;">
                    ...
                    <p class="streak" style="margin: 1rem 0 1.5rem; font-size: 0.85rem; font-family: 'JetBrains Mono', monospace; color: var(--text-muted); text-transform: uppercase;">
                        Streak: <strong style="color: var(--primary); font-size: 1.2rem; margin-left: 0.5rem;"><?= (int)($h['streak'] ?? 0) ?></strong>
                    </p>
                    <button type="button" class="check-btn habit-check-btn" data-id="<?= (int)$h['id'] ?>" data-csrf="<?= e(Core\Csrf::token()) ?>" style="width: 100%; padding: 0.75rem; border-radius: var(--radius-sm); border: 1px solid var(--primary); background: transparent; color: var(--primary); font-weight: 800; text-transform: uppercase; font-size: 0.75rem; cursor: pointer;">
                        Mark Done Today
                    </button>
                </article>
            <?php endforeach; ?>
